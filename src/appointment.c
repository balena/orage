...
fill_appt(appt_data *appt, appt_win *apptw)
{
    ...
    starttime = gtk_entry_get_text(GTK_ENTRY(GTK_BIN(apptw->appStartTime_comboboxentry)->child));
    ...
    endtime = gtk_entry_get_text(GTK_ENTRY(GTK_BIN(apptw->appEndTime_comboboxentry)->child));
...
{
    gboolean ok = FALSE;
    appt_data *appt = xfical_app_alloc();
...
on_appStartEndDate_clicked_cb(GtkWidget *button, gpointer *user_data)
{
    ...
    selDate_Window_dialog = gtk_dialog_new_with_buttons(
                            NULL, GTK_WINDOW(apptw->appWindow),
...
fill_appt_window(appt_win *apptw, char *action, char *par)
{
  ...
  /* par contains XFICAL_APP_DATE_FORMAT (yyyymmdd) date for NEW appointment */
	...
	gtk_text_buffer_set_text(tb, (appt_data->note ? (const gchar *) appt_data->note : ""), -1);

    if (appt_data->alarmtime != -1){
      gtk_combo_box_set_active(GTK_COMBO_BOX(apptw->appAlarm_combobox)
                   , appt_data->alarmtime);
    }
    gtk_entry_set_text(GTK_ENTRY(apptw->appSound_entry), (appt_data->sound ? appt_data->sound : ""));
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(apptw->appSoundRepeat_checkbutton), appt_data->alarmrepeat);
...
*create_appt_win(char *action, char *par, eventlist_win *event_list)
{
    ...
    /*
    This is not probably a good idea. and we need the parent window anyway.
    if (apptw->eventlist != NULL) {
      gtk_window_set_transient_for(GTK_WINDOW(apptw->appWindow), *parent);
